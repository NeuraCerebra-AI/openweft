...
import { hasActionableCheckpointWork, hasActionableUnfinishedWork } from '../../src/state/recovery.js';
...
describe('checkpoint recovery helpers', () => {
  ...
  it('hasActionableCheckpointWork ignores pending queue entries and only reflects checkpoint state', () => {
    const checkpoint = createEmptyCheckpoint({
      orchestratorVersion: '0.1.0',
      configHash: 'sha256:test',
      runId: 'run-1',
    ...
    expect(hasActionableCheckpointWork(null)).toBe(false);
    expect(hasActionableCheckpointWork(checkpoint)).toBe(false);
    ...
    checkpoint.features['001'].status = 'planned';
    expect(hasActionableCheckpointWork(checkpoint)).toBe(true);
